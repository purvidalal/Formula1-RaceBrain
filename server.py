...
async def get_recommendations(request: Request):
    ...
    result = agent_manager.get_recommendations(
        lap=data.get("lap", 18),
        tyre_age=data.get("tyre_age", 13),
        compound=data.get("compound", "MEDIUM"),
        position=data.get("position", 4),
        pitted=data.get("pitted", False)
    )
    return result
# ============================================================
#  Run the server
...
# Add a route for test page
@app.get("/test", response_class=HTMLResponse)
async def test_page():
    with open("frontend/test.html", "r") as f:
        return HTMLResponse(f.read())
@app.post("/api/game/start")
async def start_game(request: Request):
    data = await request.json()
    player_team = data.get("team", "Ferrari")
    race = create_default_race(player_team)
    race_id = str(random.randint(1000, 9999))
    active_races[race_id] = race
    return {"race_id": race_id, "state": race.to_dict(), "player_team": player_team}
...
async def take_action(request: Request):
    ...
    action = data.get("action", "STAY_OUT")
    ...
    for car in race.cars:
        if car.team != player_team:
            ...
            ai_action = get_ai_recommendation(car, race, team_profile.ai_style)
            ai_actions[car.team] = ai_action
    
    player_action = {"team": player_team, "action": action}
    race = simulate_lap(race, player_action, ai_actions)
    return {"race_id": race_id, "state": race.to_dict()}

@app.get("/api/game/recommendations/{race_id}/{team}")
async def get_game_recommendations(race_id: str, team: str):
    if race_id not in active_races:
        raise HTTPException(status_code=404, detail="Race not found")
    ...
    result = agent_manager.get_recommendations(
        ...
        pitted=car.has_pitted
    )
    return result
if __name__ == "__main__":
    import uvicorn
    print("\n" + "="*60)
    print("🏎️  RACEBRAIN SERVER STARTING")
    print("="*60)
